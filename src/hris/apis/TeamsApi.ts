...
export interface TeamsListRequest {
    ...
    createdBefore?: Date;
    cursor?: string;
    expand?: Array<TeamsListExpandEnum>;
...
export interface TeamsRetrieveRequest {
    id: string;
    expand?: Array<TeamsRetrieveExpandEnum>;
    includeRemoteData?: boolean;
}
...
export class TeamsApi extends runtime.BaseAPI {
    ...
    async teamsRetrieveRaw(requestParameters: TeamsRetrieveRequest): Promise<runtime.ApiResponse<Team | undefined>> {
        ...
        const response = await this.request({
            path: `/hris/v1/teams/{id}`.replace(`{${"id"}}`, encodeURIComponent(String(requestParameters.id))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });
...
* @enum {string}
*/
export enum TeamsListExpandEnum {
...
* @enum {string}
*/
export enum TeamsRetrieveExpandEnum {
    ParentTeam = 'parent_team'
}
