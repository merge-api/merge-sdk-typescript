...
import { PaginatedIncomeStatementList } from '../model/paginatedIncomeStatementList';
...
export enum IncomeStatementsApiApiKeys {
    accountTokenAuth,
}

export class IncomeStatementsApi {
    ...
    public setApiKey(key: IncomeStatementsApiApiKeys, value: string) {
        (this.authentications as any)[IncomeStatementsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async incomeStatementsList (createdAfter?: Date, createdBefore?: Date, cursor?: string, includeDeletedData?: boolean, includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedIncomeStatementList;  }> {
        const localVarPath = this.basePath + '/income-statements';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedIncomeStatementList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedIncomeStatementList");
     ...
     * @param includeRemoteData Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async incomeStatementsRetrieve (id: string, includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: IncomeStatement;  }> {
        const localVarPath = this.basePath + '/income-statements/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling incomeStatementsRetrieve.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: IncomeStatement;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "IncomeStatement");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
