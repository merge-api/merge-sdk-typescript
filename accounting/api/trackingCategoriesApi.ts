...
import { PaginatedTrackingCategoryList } from '../model/paginatedTrackingCategoryList';
...
export enum TrackingCategoriesApiApiKeys {
    accountTokenAuth,
}

export class TrackingCategoriesApi {
    ...
    public setApiKey(key: TrackingCategoriesApiApiKeys, value: string) {
        (this.authentications as any)[TrackingCategoriesApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async trackingCategoriesList (createdAfter?: Date, createdBefore?: Date, cursor?: string, includeDeletedData?: boolean, includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedTrackingCategoryList;  }> {
        const localVarPath = this.basePath + '/tracking-categories';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedTrackingCategoryList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedTrackingCategoryList");
     ...
     * @param includeRemoteData Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async trackingCategoriesRetrieve (id: string, includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TrackingCategory;  }> {
        const localVarPath = this.basePath + '/tracking-categories/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling trackingCategoriesRetrieve.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TrackingCategory;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "TrackingCategory");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
