...
import { PaginatedCashFlowStatementList } from '../model/paginatedCashFlowStatementList';
...
export enum CashFlowStatementsApiApiKeys {
    accountTokenAuth,
}

export class CashFlowStatementsApi {
    ...
    public setApiKey(key: CashFlowStatementsApiApiKeys, value: string) {
        (this.authentications as any)[CashFlowStatementsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async cashFlowStatementsList (createdAfter?: Date, createdBefore?: Date, cursor?: string, includeDeletedData?: boolean, includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedCashFlowStatementList;  }> {
        const localVarPath = this.basePath + '/cash-flow-statements';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedCashFlowStatementList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedCashFlowStatementList");
     ...
     * @param includeRemoteData Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async cashFlowStatementsRetrieve (id: string, includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CashFlowStatement;  }> {
        const localVarPath = this.basePath + '/cash-flow-statements/{id}'
        ...
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling cashFlowStatementsRetrieve.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CashFlowStatement;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "CashFlowStatement");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
